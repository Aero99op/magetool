...
const SCALE_OPTIONS = [{ value: 2, label: '2x (Double)' }, { value: 4, label: '4x (Quadruple)' }, { value: 8, label: '8x (Maximum)' }];
...
export default function AIUpscalerClient() {
    ...
    const handleFilesSelected = useCallback(async (files: File[]) => {
        ...
        try {
            const response = await imageApi.upscale(file, scale, (e) => { if (e.total) setProgress(Math.round((e.loaded / e.total) * 100)); });
        ...
        <ToolLayout
            ...
            configPanel={
                <div>
                    ...
                    {stage !== 'idle' && <button onClick={() => { setStage('idle'); setDownloadReady(false); }} className="btn btn-ghost" style={{ width: '100%', marginTop: '16px' }}>Upscale Another</button>}
                </div>
            }
            ...
            toolContent={<ToolContent overview="Enhance low-resolution images with AI. Increase resolution up to 8x." features={["AI Enhancement: Reconstructs details.", "Up to 8x Zoom: Scale small images.", "Noise Reduction: Cleans artifacts.", "Fast Processing: Results in seconds."]} howTo={[{ step: "Upload Image", description: "Select low-res photo." }, { step: "Select Scale", description: "Choose 2x, 4x, or 8x." }, { step: "Upscale", description: "AI enhances image." }, { step: "Download", description: "Save high-res version." }]} />}
        />
    );
}
